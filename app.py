...
<style>
    .main-header {
        ...
        padding: 20px 30px; border-radius: 12px; margin-bottom: 24px; color: white;
    }
...
<div class="main-header">
  ...
  <p>Man Hours &middot; Units Per Hour &middot; Occupation Levels &middot; Daily &amp; Weekly Tracking</p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    if mh is None:
        ...
        with c2: st.markdown(f'<div class="metric-card"><h3>Trainees</h3><p>{lc.get("Trainee",0)}</p></div>', unsafe_allow_html=True)
        with c3: st.markdown(f'<div class="metric-card"><h3>Starters</h3><p>{lc.get("Starter",0)}</p></div>', unsafe_allow_html=True)
        with c4: st.markdown(f'<div class="metric-card"><h3>Competent</h3><p>{lc.get("Competent",0)}</p></div>', unsafe_allow_html=True)
        with c5: st.markdown(f'<div class="metric-card"><h3>Masters</h3><p>{lc.get("Master",0)}</p></div>', unsafe_allow_html=True)

        st.markdown('<div class="section-header">Employees by Company</div>', unsafe_allow_html=True)
        comp_df = mh.groupby("Company Name").agg(
            Employees=("Name Surname","count"), Avg_Weeks=("Weeks On Site","mean")
        ...
        if not uph.empty:
            ...
            uph_sum = uph.groupby("Report Type").agg(
                Workers=("username","count"), Avg_UPH=("UPH","mean"),
                Max_UPH=("UPH","max"), Min_UPH=("UPH","min"), Total_Units=("Total Units","sum")
            ).reset_index()
...
with tab4:
    ...
    if uph.empty:
        ...
        disp_uph.index += 1
        show = disp_uph[["Name Surname","username","Company","Department","Occupation",
                          "Occupation Level","Weeks On Site","Report Type","Total Units","UPH"]].copy()
...
with tab5:
    ...
    if not rate_dfs:
        ...
        if rate_df is None:
            ...
            with col_i3:
                st.markdown(f'<div class="metric-card"><h3>Target</h3><p style="font-size:1.1rem">{target_str if target_str else "N/A"}</p></div>', unsafe_allow_html=True)
            ...
            for _, row in rate_df.iterrows():
                ...
                total_units = row["Total"]
                name = username; company = "-"; dept = "-"
                occupation = "-"; hire_date = "-"; weeks = "-"
                level = "-"; years_svc = "-"
            ...
            st.markdown("---")
            st.markdown('<div class="section-header">Select a Worker to See Hourly Breakdown</div>', unsafe_allow_html=True)
            ...
            if selected_u != "-- Select --":
                ...
                if hour_label_cols:
                    ...
                    uh2.metric("Active Hours",  user_row["Active Hours"])
                    uh3.metric("Avg UPH",       user_row["Avg UPH"])
                    peak_h = hour_df.loc[hour_df["Units"].idxmax(), "Hour"] if not hour_df["Units"].eq(0).all() else "-"
                    ...
                    active_only = hour_df[hour_df["Units"] > 0].reset_index(drop=True)
                    st.dataframe(active_only, use_container_width=True, hide_index=True)
